...
def process_image(img):
    ...
    if new_w % 2 != 0 and new_h % 2 == 0:
        top, bottom, left, right = int((min_side - new_h) / 2), int((min_side - new_h) / 2), int(
            (min_side - new_w) / 2 + 1), int((min_side - new_w) / 2)
    elif new_h % 2 != 0 and new_w % 2 == 0:
        top, bottom, left, right = int((min_side - new_h) / 2 + 1), int((min_side - new_h) / 2), int(
...
def showResults(path, borders, results=None):
    ...
    for i, border in enumerate(borders):
        cv2.rectangle(img, border[0], border[1], (0, 0, 255), 3)
        if results:
...
class picture(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("Calculating Judge system")

        self.label = QLabel(self)
        self.label.setText("   原始图片")
        self.label.setFixedSize(900, 450)
        self.label.move(50, 100)
        self.label.setStyleSheet("QLabel{background:white;}"
                                 "QLabel{color:rgb(255,255,255);font-size:50px;font-weight:bold;font-family:宋体;}"
                                 )

        self.label2 = QLabel(self)
        self.label2.setText("   批改后的图片")
        self.label2.setFixedSize(900, 450)
        self.label2.move(50,560)
        self.label2.setStyleSheet("QLabel{background:white;}"
                                 "QLabel{color:rgb(255,255,255);font-size:50px;font-weight:bold;font-family:宋体;}"
                                 )

        btn = QPushButton(self)
        btn.setText("Input Image")
        btn.move(10, 30)
        ...
        btn.setText("Judge")
        btn.move(150, 30)
        btn.clicked.connect(self.judge)
    ...
    def openimage(self):
        ...
        for x in range(len(borders)):
            ...
            if x > 0:
                ...
                if (borders[x][0][1] != borders[x - 1][0][1]) or (
                        interval_between > 2 * (interval_all - interval_between)):
                    num += 1
        ...
        while i < len(mat_labels):
            ...
            try:
                ...
                jpg = QtGui.QPixmap('./sorry.jpg').scaled(self.label.width(), self.label.height())
        ...
        if len(Judge) > 0:
            ...
            for j in Judge:
                ...
                while count < len(mat_labels) and j == mat_labels[count]:
                    count += 1
                count2 = count
                border_temp = [(borders[count1][0][0], borders[count1][0][1]),
                               (borders[count2 - 1][1][0], borders[count2 - 1][1][1])]
